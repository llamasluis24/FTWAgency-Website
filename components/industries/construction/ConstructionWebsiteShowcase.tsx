...
import { ArrowRight, ArrowUpRight, Check, HardHat, Phone } from "lucide-react";
import { motion } from "framer-motion";
import { SectionHeading } from "@/components/ui/SectionHeading";
import { Section, Container } from "@/components/layout/Section";
import { Reveal } from "@/components/ui/Reveal";
import type { constructionWebsiteModules } from "@/content/industries/modules/construction-website-modules";
import { cn } from "@/lib/utils";

type Project = (typeof constructionWebsiteModules.showcase.projects)[number];
    ...
    <div className="flex items-center justify-between gap-4 border-b border-slate-200/80 bg-white px-4 py-3 sm:px-6">
      ...
      <nav className="hidden items-center gap-5 text-[11px] font-medium text-slate-600 md:flex">
        {["Services", "Portfolio", "Process", "Reviews", "Contact"].map((item) => (
          <span key={item} className="cursor-default hover:text-sky-600">
            {item}
          </span>
        ))}
      </nav>
      <button
        type="button"
        className="inline-flex items-center gap-1.5 rounded-lg bg-sky-600 px-3 py-2 text-[11px] font-semibold text-white"
      >
...
function SiteHero({
  ...
  hero: typeof constructionWebsiteModules.showcase.hero;
}) {
  ...
  return (
    <div className="relative overflow-hidden bg-white">
      ...
      />
      <div
        className="absolute inset-0 bg-gradient-to-r from-white from-40% via-white/90 to-white/55"
        aria-hidden
      />
      ...
      <div className="relative px-4 py-8 sm:px-8 sm:py-10">
        ...
        <h3 className="mt-4 max-w-lg font-display text-2xl font-bold leading-[1.15] tracking-tight !text-slate-950 sm:text-[1.75rem]">
          {parts[0]}
          <span className="bg-gradient-to-r from-sky-600 to-cyan-500 bg-clip-text font-bold !text-transparent">
            {hero.headlineAccent}
          </span>
          {parts[1]}
        </h3>
  ...
  return (
    <motion.article
      ...
      className={cn(
        ...
        project.layout === "large" && "col-span-2 row-span-2 sm:col-span-1 sm:min-h-[280px]",
        project.layout === "tall" && "col-span-1 row-span-2 sm:min-h-[280px]",
        project.layout === "medium" && "col-span-1 row-span-1 min-h-[130px]",
      )}
    >
      ...
      <div className="absolute inset-x-0 bottom-0 p-4">
        ...
        <p className="mt-0.5 font-display text-sm font-semibold text-white">{project.name}</p>
        <p className="tnum mt-1 text-xs font-medium text-sky-300">{project.value}</p>
      </div>
    </motion.article>
  );
}
...
function ProjectGallery({
  ...
  gallery: typeof constructionWebsiteModules.showcase.gallery;
  projects: Project[];
}) {
  return (
...
export function ConstructionWebsiteShowcase({
  content,
}: {
  content: typeof constructionWebsiteModules.showcase;
}) {
  return (
    <Section surface>
      <Container>
        ...
        <Reveal>
          <div className="mx-auto max-w-4xl overflow-hidden rounded-2xl border border-white/10 bg-white text-slate-900 shadow-[0_0_48px_rgba(0,212,255,0.08)] [&_h3]:!text-slate-950 [&_h4]:!text-slate-950">
            ...
            <ProjectGallery gallery={content.gallery} projects={[...content.projects]} />

            <div className="grid grid-cols-3 gap-3 border-t border-slate-200 bg-white px-4 py-5 sm:px-8">
              {content.stats.map((stat) => (
                <div key={stat.label} className="text-center">
                  <p className="tnum font-display text-xl font-bold text-sky-600">{stat.value}</p>
