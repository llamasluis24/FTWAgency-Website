...
import type { ProfessionalServicesAioModules } from "@/content/industries/modules/professional-services-aio-modules";
...
function TrendChart({
  ...
  const gradId = `aio-ps-${label.replace(/\s+/g, "-").toLowerCase()}`;
...
export function ProfessionalServicesAioVisibilityChart({
  content,
}: {
  content: ProfessionalServicesAioModules["visibility"];
}) {
  return (
    <Section>
      <Container>
        ...
        <Reveal className="mx-auto max-w-5xl overflow-hidden rounded-2xl border border-white/10 bg-bg shadow-[0_0_48px_rgba(0,212,255,0.08)]">
          <div className="flex items-center justify-between border-b border-white/10 bg-elevated px-4 py-3 sm:px-5">
            <div className="flex items-center gap-2">
              ...
              <span className="ml-2 hidden text-xs text-muted sm:inline">
                meridianlawgroup.com · AIO Analytics
              </span>
            </div>
          ...
          <div className="space-y-6 bg-white p-4 sm:p-6">
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-4">
              <p className="mb-4 text-xs font-medium text-slate-500">Top Cited Practice Pages</p>
