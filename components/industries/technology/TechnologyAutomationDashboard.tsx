...
function TrendChart({
  ...
  const w = 360;
  const h = 120;
  const displayValues = invert ? [...values].reverse() : values;
  const area = seriesToArea(displayValues, w, h);
  const line = seriesToPath(displayValues, w, h);
  const gradId = `auto-tech-${label.replace(/\s+/g, "-").toLowerCase()}`;
...
export function TechnologyAutomationDashboard({
  content,
}: {
  content: TechnologyAutomationModules["performance"];
}) {
  return (
    <Section>
      <Container>
          ...
          <div className="flex items-center justify-between border-b border-white/10 bg-elevated px-4 py-3 sm:px-5">
            <div className="flex items-center gap-2">
              ...
              <span className="ml-2 hidden text-xs text-muted sm:inline">
                clearpath.io · Automation Analytics
              </span>
            </div>
            <span className="rounded-full border border-amber-400/20 bg-amber-400/10 px-2.5 py-0.5 text-[10px] font-medium uppercase tracking-wider text-amber-300">
              Live Preview
            </span>
          </div>
          ...
          <div className="space-y-6 bg-white p-4 sm:p-6">
            ...
            <div className="grid gap-4 lg:grid-cols-3">
              <TrendChart
                ...
                values={[...content.responseTrend]}
                color="#f59e0b"
                invert
              />
              <TrendChart
                label="Trials Nurtured / mo"
                values={[...content.nurtureTrend]}
                color="#0891b2"
              />
              <TrendChart
                label="Trial → Demo Rate (%)"
                values={[...content.conversionTrend]}
                color="#059669"
              />
            </div>
            ...
            <div className="grid gap-4 lg:grid-cols-2">
              <div className="rounded-xl border border-slate-200 bg-white p-4">
                <p className="mb-4 text-xs font-medium text-slate-500">Automation Mix</p>
                <div className="space-y-3">
                  {content.workflowBreakdown.map((p) => (
                    <div key={p.name}>
              ...
              <div className="rounded-xl border border-slate-200 bg-white p-4">
                <p className="mb-4 text-xs font-medium text-slate-500">Top Active Workflows</p>
                <ul className="space-y-2">
                  {content.topAutomations.map((item, i) => (
                    <li
                      key={item.name}
                      className="flex items-center justify-between rounded-lg border border-slate-200 bg-slate-50 px-3 py-2"
                    >
                      <span className="flex items-center gap-2 text-xs text-slate-700">
                        <span className="tnum flex h-5 w-5 items-center justify-center rounded-full bg-amber-100 text-[10px] font-bold text-amber-700">
                          {i + 1}
                        </span>
                        {item.name}
                      </span>
                      <span className="tnum text-xs text-emerald-600">{item.runs} runs</span>
                    </li>
                  ))}
                </ul>
              </div>
